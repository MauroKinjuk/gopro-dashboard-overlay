...
class LeaderboardVideoGenerator:
    ...
    def generate_leaderboard_frame(self, segment: Dict, frame: int, 
                                   total_frames: int, phase: str = "static",
                                   show_position: bool = False) -> Image.Image:
        """Genera frame del leaderboard
        
        Args:
            segment: Datos del segmento
            frame: Número de frame actual
            total_frames: Total de frames en esta fase
            phase: 'building' (tabla apareciendo) o 'position' (mostrando posición)
            show_position: Si True, resalta/animar la posición del usuario
        """
        img = Image.new('RGBA', (self.width, self.height), (0, 0, 0, 0))
        draw = ImageDraw.Draw(img)
        ...
        # Fase "building": animar entrada de filas
        max_visible_rows = len(leaderboard)
        if phase == "building":
            # Aparece gradualmente: 0% al inicio, 100% al final
            max_visible_rows = int(len(leaderboard) * progress)
            max_visible_rows = max(1, max_visible_rows)  # Al menos 1 fila visible
        
        # Mostrar top 10 (o menos durante la fase de building)
        entries_to_show = min(len(leaderboard), 10, max_visible_rows)
        
        # Calcular qué fila está animando (para fase "position")
        highlight_row = -1
        if phase == "position" and show_position and my_pos:
            # Animar ciclo a través de las filas hasta llegar a la posición del usuario
            cycle_duration = 0.3  # 30% del tiempo para cada ciclo
            cycle_position = (progress % cycle_duration) / cycle_duration
            rows_to_highlight = int(my_pos * cycle_position) if my_pos <= 10 else 10
            highlight_row = min(rows_to_highlight, entries_to_show - 1)
        ...
        for i in range(entries_to_show):
            ...
            # Durante fase "position", resaltar la fila animada (pero NO es "yo", solo animación)
            is_highlighted = False
            if phase == "position" and show_position:
                if i == highlight_row:
                    is_highlighted = True
            
            if is_me:
                row_bg = (80, 60, 20, 240)  # Color para "TÚ" (marrón/dorado)
            elif is_highlighted:
                row_bg = (120, 90, 30, 250)  # Color de animación (naranja más brillante)
        ...
        if my_pos and my_pos > 10:
            show_my_row = True
            if phase == "building":
                # Aparece al final de la animación de building
                show_my_row = progress > 0.8
            ...
            if show_my_row:
                ...
                # Animación de entrada para tu fila
                my_row_bg = (100, 80, 30, 250)
                if phase == "position" and show_position:
                    # Pulso de brillo
                    pulse = abs((progress * 4) % 2 - 1)  # 0->1->0
                    alpha_boost = int(50 * pulse)
                    my_row_bg = (100 + alpha_boost//3, 80 + alpha_boost//3, 30 + alpha_boost//5, 250)
    ...
    def generate_closing_frame(self, segment: Dict, progress: float) -> Image.Image:
        """Genera frame de cierre - desvanecimiento"""
        img = Image.new('RGBA', (self.width, self.height), (0, 0, 0, 0))
        draw = ImageDraw.Draw(img)
        
        # Panel principal con opacidad decreciente
        alpha = int(230 * (1 - progress))
        bg_color = (*self.BG_COLOR[:3], alpha)
        ...
        # Nombre del segmento (más pequeño, centrado)
        name = segment.get('name', 'Segmento')[:40]
        font_header = self.get_font(20)
        bbox = draw.textbbox((0, 0), name, font=font_header)
        name_w = bbox[2] - bbox[0]
        name_x = margin + (panel_w - name_w) // 2
        name_y = margin + panel_h // 2 - 15
        
        header_color = (*self.TEXT_COLOR[:3], alpha)
        draw.text((name_x, name_y), name, font=font_header, fill=header_color)
        
        # Mensaje de cierre
        font_msg = self.get_font(16)
        msg = "¡Segmento completado!"
        bbox = draw.textbbox((0, 0), msg, font=font_msg)
        msg_w = bbox[2] - bbox[0]
        msg_x = margin + (panel_w - msg_w) // 2
        msg_y = name_y + 35
        
        draw.text((msg_x, msg_y), msg, font=font_msg, fill=header_color)
        
        return img
    
    def generate_video(self, segment: Dict, output_file: Path):
        """Genera video completo para un segmento con 4 fases:
        0-2s: Intro "Se aproxima el segmento"
        2-6s: Tabla apareciendo (4s)
        6-12s: Animación de posición del usuario (6s)
        12s: Cierre
        """
        
        # Nuevos tiempos: 12s total
        intro_duration = 2.0           # 0-2s: Intro
        table_build_duration = 4.0    # 2-6s: Tabla apareciendo
        position_duration = 6.0       # 6-12s: Mostrar posición
        closing_duration = 0.5        # Último medio segundo: cierre
        
        intro_frames = int(intro_duration * self.fps)
        table_build_frames = int(table_build_duration * self.fps)
        position_frames = int(position_duration * self.fps)
        closing_frames = int(closing_duration * self.fps)
        
        # Ajustar para que coincida con total_frames
        total_calculated = intro_frames + table_build_frames + position_frames + closing_frames
        if total_calculated < self.total_frames:
            position_frames += (self.total_frames - total_calculated)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            log(f"  Fases: Intro({intro_frames}) + Tabla({table_build_frames}) + Posición({position_frames}) + Cierre({closing_frames})")
            ...
            for i in range(intro_frames):
                ...
                if i % 10 == 0:
                    log(f"  Frame {i}/{self.total_frames} [Intro]")

            # Fase 2: Tabla apareciendo (2-6s)
            base_frame = intro_frames
            for i in range(table_build_frames):
                frame_num = base_frame + i
                frame = self.generate_leaderboard_frame(segment, i, table_build_frames, 
                                                       phase="building", show_position=False)
                frame.save(tmp_path / f"frame_{frame_num:05d}.png")
                if i % 20 == 0:
                    log(f"  Frame {frame_num}/{self.total_frames} [Tabla]")

            # Fase 3: Animación de posición (6-12s)
            base_frame = intro_frames + table_build_frames
            for i in range(position_frames):
                frame_num = base_frame + i
                frame = self.generate_leaderboard_frame(segment, i, position_frames,
                                                       phase="position", show_position=True)
                frame.save(tmp_path / f"frame_{frame_num:05d}.png")
                if i % 30 == 0:
                    log(f"  Frame {frame_num}/{self.total_frames} [Posición]")

            # Fase 4: Cierre (últimos frames)
            base_frame = intro_frames + table_build_frames + position_frames
            for i in range(closing_frames):
                frame_num = base_frame + i
                progress = i / closing_frames if closing_frames > 0 else 1.0
                frame = self.generate_closing_frame(segment, progress)
                frame.save(tmp_path / f"frame_{frame_num:05d}.png")
...
def main():
    ...
    parser.add_argument("--duration", type=float, default=12.0,
                        help="Duración de cada video en segundos (default: 12s para 4 fases de animación)")
