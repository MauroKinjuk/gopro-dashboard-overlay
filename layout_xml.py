...
from .widgets.segment_overlay import SegmentOverlayWidget
...
def elapsed_formatter_from(entry: Callable[[], Entry], format_string: str) -> Callable[[], str]:
    """
    Formats elapsed time from milliseconds to HH:MM:SS or MM:SS depending on duration.
    format_string can be 'short' (MM:SS) or 'long' (HH:MM:SS)
    """
    def format_elapsed():
        e = entry()
        if e is None or e.timestamp is None:
            return "--:--"
        timestamp_ms = e.timestamp.magnitude
        total_seconds = int(timestamp_ms / 1000)
        hours = total_seconds // 3600
        minutes = (total_seconds % 3600) // 60
        seconds = total_seconds % 60
        if hours > 0 or format_string == "long":
...
class Widgets:
    ...
    @allow_attributes({"x", "y", "size", "format", "align", "cache", "rgb", "outline", "outline_width"})
    def create_elapsed(self, element, entry, **kwargs):
        format_string = attrib(element, "format", d="short")
        return text(
            at=at(element),
            value=elapsed_formatter_from(entry, format_string),
            font=self._font(element, "size", d=16),
            align=attrib(element, "align", d="left"),
            cache=battrib(element, "cache", d=False),
            fill=rgbattr(element, "rgb", d=(255, 255, 255)),
            stroke=rgbattr(element, "outline", d=(0, 0, 0)),
            stroke_width=iattrib(element, "outline_width", d=2)
        )
    ...
    @allow_attributes({"x", "y", "file", "size", "width", "mode"})
    def create_segment_overlay(self, element: ET.Element, entry, **kwargs) -> Widget:
        """Widget para mostrar información de segmentos de Strava"""
        from pathlib import Path
        segment_file = Path(attrib(element, "file"))
        return SegmentOverlayWidget(
            segment_data_file=segment_file,
            entry_provider=entry,
            font=self.font,
            position=at(element),
            width=iattrib(element, "width", d=350),
            mode=attrib(element, "mode", d="normal")
        )
