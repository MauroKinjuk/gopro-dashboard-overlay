...
def gopro_dashboard_arguments(args=None):
    ...
    gpx.add_argument("--segment-data", type=pathlib.Path,
                     help="JSON file with Strava segment data (from segment_matcher.py)")
    ...
    only.add_argument("--video-time-end", choices=["file-created", "file-modified", "file-accessed", "video-created"],
                      help="Use file/video dates for aligning video and GPS information, only when --use-gpx-only. "
                           "'video-created' reads the creation_time embedded in the MP4 metadata.")
    only.add_argument("--start-date", type=str,
                      help="Start date/time for GPX-only mode (format: YYYY-MM-DD HH:MM:SS). Overrides video-time-start.")
    only.add_argument("--duration", type=int,
                      help="Duration in seconds for GPX-only mode. Limits output video length.")
