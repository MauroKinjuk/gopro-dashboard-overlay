...
class LayoutEditor:
    ...
    def update_properties_panel(self):
            ...
            try:
                ...
                if hasattr(self, 'add_frame') and self.add_frame is not None:
                    ...
                    for w in self.side_panel.pack_slaves():
                        if w not in (self.selection_label, self.save_btn, getattr(self, 'rerender_btn', None), getattr(self, 'undo_btn', None)) and w is not self.add_frame:
                            w.destroy()
                    self.add_frame.pack(fill=tk.X, padx=10, pady=(5, 10))
                return
            except Exception:
        ...
        if ctype == "text":
            ...
            # Colores de la barra: crear un contenedor y usar create_color_control
            ...
            align_options = ["left", "right", "lt", "mt", "rt", "lm", "mm", "rm", "lb", "mb", "rb"]
    ...
    def add_component(self, ctype: str, x: int = 0, y: int = 0):
        ...
        try:
            ...
            # Determinar contenedor (si hay translate seleccionado, usarlo)
